...
export function quoteLinkError(value: string, touched: boolean): string | null {
  if (!touched || !value.trim()) return null;
  if (!isValidXLink(value)) {
    return "Paste the full link from the post (x.com or twitter.com).";
  }
  return null;
}
