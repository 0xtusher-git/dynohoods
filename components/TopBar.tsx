...
import { useWhitelist } from "@/components/WhitelistModal";
    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-background/95 backdrop-blur-sm">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-3 px-5 sm:px-8">
        ...
        <div className="flex items-center gap-2.5">
          {/* contract-address pill — only once deployed */}
          {hasContract && (
            <button
          ...
          {/* marketplace CTA — once the listing is live */}
          {marketplaceLive ? (
            <a
            ...
            <button onClick={open} className="btn btn-primary px-4 py-2 text-sm">
              Get whitelisted
            </button>
          )}
        </div>
